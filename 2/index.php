...
function hn_ts_upload_measurement_file($tablename, $data, $filename, $timestamp){
	...
	if(file_put_contents("../../../..$fp", $data)){
		hn_ts_add_measurement($tablename, SITEPATH.$fp, $timestamp);
	}else{
...
function hn_ts_get_filepath_for_table($tablename, $filename){
	...
	if(MULTISITE){
		$basepath = "/wp-content/blogs.dir/".$blogid."/files/";
	}else{
	...
	//presumes location of this file is  .../wp-content/plugins/timestreams/2/index.php
	$basepath1='../../../..';
	$dir = $basepath1.$basepath;
	while(file_exists($basepath1.$basepath.$prefix.$filename)){
		if($prefix === ""){
			$prefix = 0;
		}else{
			$prefix = $prefix + 1;
	...
	if(!is_writable($dir)){
		if(!mkdir($dir, 0755,true)){
			if(HN_TS_DEBUG){
				hn_ts_error_msg("Failed to access or create folder:.$dir", 400);
			}else{
				hn_ts_error_msg("Failed to access or create folder.", 400);
			}
		}
	}
